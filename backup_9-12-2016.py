...
def textfile2summary(fileName):
    ...
    #--------------------------------
    '''
    SM_List=numpy.array(ScoreMatList)
    S_arr=ScoreMatList.max(axis=1)
    max_val=max(S_att)

    mat_len=len(ScoreMatList)
    
    for i in range(mat_len):
        for j in range(ScoreMatList[i]):
            ScoreMatList[i][j]=max_val-ScoreMatList[i][j]
    '''
...
#-------------------------------main ---------------------------------------
DocumentName="African runner murder(8)"
